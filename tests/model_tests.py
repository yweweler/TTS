...
class VoiceLoopConvTests(unittest.TestCase):
    def test_in_out(self):
        ...
        assert align.shape[0] == 4, "size is not {} but {}".format(4, align.shape[0])
        assert align.shape[1] == 24,  "size is not {} but {}".format(24, align.shape[1])
        assert align.shape[2] == 100,  "size is not {} but {}".format(120, align.shape[2])