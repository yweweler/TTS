...
class VoiceLoopConv(nn.Module):
    def __init__(self, embedding_dim=256, linear_dim=1025, mel_dim=80, r=5):
        ...
        self.encoder = EncoderConv(
            len(symbols), embedding_dim, embedding_dim, embedding_dim)
        self.decoder = DecoderConvWithBuffer(
            in_dim=embedding_dim, out_dim=mel_dim*r, hidden_dim=embedding_dim, buffer_len=17, memory_dim=mel_dim)
        self.last_linear = nn.Sequential(
            nn.Linear(mel_dim, 256), nn.ReLU(), nn.Linear(256, linear_dim))
